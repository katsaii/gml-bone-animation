...
if(is_real(sk_tuple)&&ds_exists(sk_tuple,ds_type_list)){
	var sk_tuple_last = ds_list_size(sk_tuple), sk_tuple_bone, sk_tuple_parent_id, sk_tuple_parent;
	for(var sk_tuple_id = 0; sk_tuple_id < sk_tuple_last; sk_tuple_id+=sk_tuple_boneParent_ENTRIES){
		sk_tuple_bone = sk_tuple[| sk_tuple_id+sk_tuple_boneParent_bone];
		sk_tuple_parent_id = sk_tuple[| sk_tuple_id+sk_tuple_boneParent_parent];
		if(sk_tuple_parent_id < sk_tuple_id){
			// valid parent
			sk_tuple_parent = sk_tuple[| sk_tuple_parent_id+sk_tuple_boneParent_bone];
