...
/// @param constraint
var sk_tuple = ds_list_find_value(argument0[SK_CONSTRAINT_HIERARCHY.boneParentTuples],argument0[SK_CONSTRAINT_HIERARCHY.tupleIdFinal]);
	...
	for(sk_tuple_id = 0; sk_tuple_id < sk_tuple_last; sk_tuple_id+=sk_tuple_boneParent_ENTRIES){
		...
		if(sk_tuple_parent_id < sk_tuple_id){
			...
			if(sk_struct_exists(sk_tuple_bone,sk_type_bone)&&sk_struct_exists(sk_tuple_parent,sk_type_bone)){
				...
				//if(sk_tuple_bone[SK_BONE.badApplied]){sk_bone_updateAppliedTransform(sk_tuple_bone);}
