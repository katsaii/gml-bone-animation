...
for(sk_slot_id = 0; sk_slot_id < sk_slot_count; sk_slot_id++){
	...
	var sk_slot_attachment = noone;
	var sk_slot_bone = noone;
	var sk_slot_colour = $ffffff;
	var sk_slot_alpha = 1;
	switch(sk_struct_get_type(sk_slot)){
		case sk_type_slot:
			// obtain attachment and bone from slot
			sk_slot_attachment = sk_slot[SK_SLOT.attachmentFinal];
			sk_slot_bone = sk_slot[SK_SLOT.boneFinal];
			sk_slot_colour = sk_slot[SK_SLOT.colourFinal];
			sk_slot_alpha = sk_slot[SK_SLOT.alphaFinal];
		break;
		case sk_type_symbol:
			...
			sk_slot_attachment = sk_nestedSlot[SK_SLOT.attachmentFinal];
			sk_slot_bone = sk_slot[SK_SYMBOL.NESTED_BONE];
			sk_slot_colour = sk_nestedSlot[SK_SLOT.colourFinal];
			sk_slot_alpha = sk_nestedSlot[SK_SLOT.alphaFinal];
		break;
	}
	switch(sk_struct_get_type(sk_slot_attachment)){
		case sk_type_attachment_plane:
			// render region attachment
			var sk_attachment_subtexture = sk_atlas_subtextures[? sk_slot_attachment[SK_ATTACHMENT_PLANE.regionKey]];
			if(sk_struct_exists(sk_attachment_subtexture,sk_type_region)){
				...
				var sk_bone_m10 = 0;
				var sk_bone_m11 = -1;
				if(sk_struct_exists(sk_slot_bone,sk_type_bone)){
					sk_bone_x = sk_slot_bone[SK_BONE.XWorld];
					sk_bone_y = sk_slot_bone[SK_BONE.YWorld];
					sk_bone_m00 = sk_slot_bone[SK_BONE.m00]; // x component of x
					sk_bone_m01 = sk_slot_bone[SK_BONE.m01]; // y component of x
					sk_bone_m10 = sk_slot_bone[SK_BONE.m10]; // x component of y
					sk_bone_m11 = sk_slot_bone[SK_BONE.m11]; // y component of y
				}
				// get attachment transform
				var sk_att_dx = sk_slot_attachment[SK_ATTACHMENT_PLANE.X];
				var sk_att_dy = sk_slot_attachment[SK_ATTACHMENT_PLANE.Y];
				var sk_att_m00 = sk_slot_attachment[SK_ATTACHMENT_PLANE.m00]; // x component of x
				var sk_att_m01 = sk_slot_attachment[SK_ATTACHMENT_PLANE.m01]; // y component of x
				var sk_att_m10 = sk_slot_attachment[SK_ATTACHMENT_PLANE.m10]; // x component of y
				var sk_att_m11 = sk_slot_attachment[SK_ATTACHMENT_PLANE.m11]; // y component of y
				// multiply bone and attachment matrices
				var sk_region_x = sk_bone_x+sk_att_dx*sk_bone_m00+sk_att_dy*sk_bone_m10;
				var sk_region_y = sk_bone_y+sk_att_dx*sk_bone_m01+sk_att_dy*sk_bone_m11;
				...
				var sk_region_width = sk_attachment_subtexture[SK_REGION.width];
				var sk_region_height = sk_attachment_subtexture[SK_REGION.height];
				var sk_region_left = -sk_attachment_subtexture[SK_REGION.ox];
				var sk_region_top = -sk_attachment_subtexture[SK_REGION.oy];
				var sk_region_right = sk_region_left+sk_region_width;
				var sk_region_bottom = sk_region_top+sk_region_height;
				// get verts
				var v_x1 = sk_region_x+sk_region_m00*sk_region_left+sk_region_m10*sk_region_bottom; // bottom left
				var v_y1 = sk_region_y+sk_region_m01*sk_region_left+sk_region_m11*sk_region_bottom;
				var v_x2 = sk_region_x+sk_region_m00*sk_region_right+sk_region_m10*sk_region_bottom; // bottom right
				var v_y2 = sk_region_y+sk_region_m01*sk_region_right+sk_region_m11*sk_region_bottom;
				var v_x3 = sk_region_x+sk_region_m00*sk_region_left+sk_region_m10*sk_region_top; // top left
				var v_y3 = sk_region_y+sk_region_m01*sk_region_left+sk_region_m11*sk_region_top;
				var v_x4 = sk_region_x+sk_region_m00*sk_region_right+sk_region_m10*sk_region_top; // top right
				var v_y4 = sk_region_y+sk_region_m01*sk_region_right+sk_region_m11*sk_region_top;
				// add verticies to buffer
				#region // triad #1
				// bottom left
				vertex_position(argument2,v_x1,v_y1);
				vertex_normal(argument2,0,0,-1);
				vertex_colour(argument2,sk_slot_colour,sk_slot_alpha);
