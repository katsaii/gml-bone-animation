...
if(sk_struct_exists(argument1,sk_type_constraint_hierarchy)){
	...
	var sk_count = ds_list_size(sk_availableTuples);
	for(var sk_id = 0; sk_id < sk_count; sk_id++){
		var sk_tuple = sk_availableTuples[| sk_id];
		if(is_real(sk_tuple)&&ds_exists(sk_tuple,ds_type_list)){
			...
			for(sk_tuple_id = 0; sk_tuple_id < sk_tuple_last; sk_tuple_id+=sk_tuple_boneParent_ENTRIES){
				...
				sk_cache_sort_bone(argument0,sk_tuple[| sk_tuple_id+sk_tuple_boneParent_bone]);
			}
		}
	}
