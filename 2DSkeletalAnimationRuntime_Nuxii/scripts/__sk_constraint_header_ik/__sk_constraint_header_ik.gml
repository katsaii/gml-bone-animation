...
switch(sk_cmd){
	...
	case sk_structure_type_command.me_construct:
		if(sk_struct_exists(sk_structure)){
			sk_constraint_ik_set_mix(sk_structure,sk_bendDir_negative,1);
			sk_constraint_ik_set_chain(sk_structure,false);
			sk_constraint_ik_set_bone_joint(sk_structure,noone);
			sk_constraint_ik_set_bone_effector(sk_structure,noone);
